...
class StudentCreate(BaseModel):
    ...
    teacher_id: Optional[int] = Field(None, description="教师ID（可空，稍后选老师）")
...
class TeacherAssign(BaseModel):
    """选老师请求体：把学生分配给指定教师（null 表示清空）"""
    teacher_id: Optional[int] = Field(None, description="目标教师ID（null 表示清空所选老师）")
